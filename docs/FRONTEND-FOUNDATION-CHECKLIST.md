...
| --- | --- | --- | --- |
| Stack: React 19.2, Vite 8, Pixi 8, Zustand, TanStack Query, React Router v7, CSS Modules | Pass | Automated | `apps/web/package.json` lockfile |
...
| Generic card table, not 8x8 board | Pass | Automated | `game/renderer` views |
...
| Manual: pointer drag + touch on card table | Manual required | Manual | `/play/r-104` and `/play/r-8892` |
| Manual: gothic canvas stays dark in Daybreak | Manual required | Manual | Night of Bloodlines room |
...
- Visual polish of Pixi cards with art assets
